            ...
            for learning_rate in learning_rates:
                for hidden_type in hidden_types:
                    ...
                    if hidden_type == "rectified":
                        hidden_layer = mlp.RectifiedLinear(layer_name='hidden', dim=dim, irange=.1, init_bias=1.)
                    else:
                        if hidden_type == "tanh":
                            hidden_layer = mlp.Tanh(layer_name='hidden', dim=dim, irange=.1, init_bias=1.)
                        else:
                            if hidden_type == "sigmoid":
                                hidden_layer = mlp.Sigmoid(layer_name='hidden', dim=dim, irange=.1, init_bias=1.)
                    output_layer = mlp.Linear(dim=2, layer_name='y', irange=.1)

                    default = Default()
                    wdecay = WeightDecay(coeffs={'hidden': 0.0001, 'y': 0.0001})
...
if best_conf[3] == "rectified":
    hidden_layer = mlp.RectifiedLinear(layer_name='hidden', dim=best_conf[0], irange=.1, init_bias=1.)
else:
    if best_conf[3] == "tanh":
        hidden_layer = mlp.Tanh(layer_name='hidden', dim=best_conf[0], irange=.1, init_bias=1.)
    else:
        if best_conf[3] == "sigmoid":
            hidden_layer = mlp.Sigmoid(layer_name='hidden', dim=best_conf[0], irange=.1, init_bias=1.)
output_layer = mlp.Linear(dim=2, layer_name='y', irange=.1)
