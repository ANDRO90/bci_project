...
def main():
    ...
    for alpha in alphas:
        ridge.alpha = alpha
        this_scores = cross_validation.cross_val_score(ridge, feats_tr, pos_x_tr)
    ...
    for k, (train, test) in enumerate(k_fold):
        ridge_cv.fit(feats_tr[train], pos_x_tr[train])
        print("[fold {0}] alpha: {1:.5f}, score: {2:.5f}". format(k, ridge_cv.alpha_, ridge_cv.score(feats_tr[test], pos_x_tr[test])))
