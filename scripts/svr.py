...
def main():
    ...
    svr_x = svm.SVR()
    svr_x.fit(feats_tr, np.ravel(pos_x_tr))
    p_x = svr_x.predict(feats_tr)
    p_x = p_x.reshape(len(p_x), 1)
    e_x = p_x - pos_x_tr

    # train the model with the default hyperparameters: kernel='rbf' (radial basis function kernel) and penalty C=1.0
    svr_y = svm.SVR()
    svr_y.fit(feats_tr, np.ravel(pos_y_tr))
    p_y = svr_y.predict(feats_tr)
    ...
    predict_x_te = svr_x.predict(feats_te)
    predict_y_te = svr_y.predict(feats_te)
