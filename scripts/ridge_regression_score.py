...
def main():
    ...
    ridge_x = linear_model.Ridge(alpha=1e-5)
    ridge_x.fit(feats_tr, pos_x_tr)
    p_x = ridge_x.predict(feats_tr)
    p_x = p_x.reshape(len(p_x), 1)
    e_x = p_x - pos_x_tr

    ridge_y = linear_model.ElasticNet(alpha=1e-5)
    ridge_y.fit(feats_tr, pos_y_tr)
    p_y = ridge_y.predict(feats_tr)
    ...
    predict_x_te = ridge_x.predict(feats_te)
    predict_y_te = ridge_y.predict(feats_te)
