...
def main():
    ...
    for alpha in alphas:
        lasso.alpha = alpha
        this_scores = cross_validation.cross_val_score(lasso, feats_tr, pos_y_tr)
    ...
    for k, (train, test) in enumerate(k_fold):
        lasso_cv.fit(feats_tr[train], pos_y_tr[train])

        print("[fold {0}] alpha: {1:.5f}, score: {2:.5f}".format(k, lasso_cv.alpha_, lasso_cv.score(feats_tr[test], pos_y_tr[test])))
